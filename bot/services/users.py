...
async def get_first_name(session: AsyncSession, user_id: int) -> str:
    query = select(UsersOrm.first_name).filter_by(id=user_id)
...
async def get_language_code(session: AsyncSession, user_id: int) -> str:
    query = select(UsersOrm.language_code).filter_by(id=user_id)
...
async def set_language_code(
    ...
    stmt = update(UsersOrm).where(UsersOrm.id == user_id).values(language_code=language_code)
...
async def is_admin(session: AsyncSession, user_id: int) -> bool:
    query = select(UsersOrm.is_admin).filter_by(id=user_id)
...
async def set_is_admin(session: AsyncSession, user_id: int, is_admin: bool) -> None:
    stmt = update(UsersOrm).where(UsersOrm.id == user_id).values(is_admin=is_admin)
